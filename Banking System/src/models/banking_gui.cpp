...
// Views: Client Management | Employee Management | Administration
// =============================================================================
...
namespace bank_state {
    ...
    // Modal open flags  (set true to open a popup on the next frame)
...
// Thin banner used by every view for summary numbers
static void stats_bar(const char* lbl_a, const char* val_a, ImVec4 col_a,
    const char* lbl_b, const char* val_b, ImVec4 col_b,
    float bar_w)
{
    ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4{ 0.10f,0.13f,0.20f,1.f });
    ImGui::BeginChild("##stats_bar", { bar_w, 54.f }, false);
    ImGui::SetCursorPos({ 12.f, 10.f });
    ImGui::TextDisabled("%s", lbl_a);
    ImGui::SameLine();
    ImGui::PushStyleColor(ImGuiCol_Text, col_a);
    ImGui::Text("%s", val_a);
    ImGui::PopStyleColor();
    ImGui::SameLine(bar_w * 0.42f);
    ImGui::TextDisabled("%s", lbl_b);
    ImGui::SameLine();
    ImGui::PushStyleColor(ImGuiCol_Text, col_b);
    ImGui::Text("%s", val_b);
    ImGui::PopStyleColor();
    ImGui::EndChild();
    ImGui::PopStyleColor();
...
static void modal_add_client()
{
    ...
    static double init_bal = 1500.0;
    static char   err[256] = {};

    ImGui::TextDisabled("Create a new bank client account");
    ImGui::Separator(); ImGui::Spacing();

    ImGui::Text("Full Name:");    ImGui::SameLine(130.f);
    ImGui::SetNextItemWidth(210.f);
    ImGui::InputText("##c_name", name_buf, sizeof(name_buf));

    ImGui::Text("Password:");     ImGui::SameLine(130.f);
    ImGui::SetNextItemWidth(210.f);
    ImGui::InputText("##c_pass", pass_buf, sizeof(pass_buf),
        ImGuiInputTextFlags_Password);

    ImGui::Text("Init. Balance:"); ImGui::SameLine(130.f);
    ImGui::SetNextItemWidth(210.f);
    ImGui::InputDouble("##c_bal", &init_bal, 500.0, 1000.0, "$ %.2f");

    auto constraints = utils::Validation::get_ValidationStruct();
    ImGui::Spacing();
    ImGui::PushStyleColor(ImGuiCol_Text, { 0.55f,0.60f,0.70f,1.f });
    ImGui::TextWrapped("Name: %d-%d letters/spaces  |  Password: %d-%d chars, no spaces  |  Min balance: $%.0f",
        constraints.minNameSize, constraints.maxNameSize,
        constraints.minPasswordSize, constraints.maxPasswordSize,
        constraints.minBalance);
    ImGui::PopStyleColor();
    ...
    if (btn_green("Create", { 100.f,0.f })) {
        ...
        if (!utils::Validation::validateName(n))
            strncpy_s(err, sizeof(err), "Invalid name — check length and characters.", _TRUNCATE);
        else if (!utils::Validation::validatePassword(p))
            strncpy_s(err, sizeof(err), "Invalid password — 8-20 chars, no spaces.", _TRUNCATE);
        else if (!utils::Validation::validateBalance(init_bal))
            strncpy_s(err, sizeof(err), "Initial balance is below the minimum.", _TRUNCATE);
        else {
            ...
            bank_state::set_status("Client created successfully.");
...
{
    if (bank_state::open_deposit) {
        ImGui::OpenPopup("Deposit");
        bank_state::open_deposit = false;
    }
    ...
    if (idx >= 0 && idx < (int)bank_state::clients.size()) {
        ...
        ImGui::Separator(); ImGui::Spacing();
        ImGui::Text("Amount:"); ImGui::SameLine(90.f);
        ...
        if (btn_green("Deposit", { 100.f,0.f })) {
            c.deposit(amount);
            bank_state::set_status("Deposit successful.");
            amount = 500.0;
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();
...
{
    if (bank_state::open_withdraw) {
        ImGui::OpenPopup("Withdraw");
        bank_state::open_withdraw = false;
    }
    if (!ImGui::BeginPopupModal("Withdraw", nullptr,
        ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove)) return;

    static double amount = 200.0;
    static char   err[128] = {};
    int idx = bank_state::selected_client;
    ...
    if (idx >= 0 && idx < (int)bank_state::clients.size()) {
        ...
        if (btn_amber("Withdraw", { 100.f,0.f })) {
            if (amount > c.getBalance())
                ...
                c.withdraw(amount);
                bank_state::set_status("Withdrawal successful.");
                amount = 200.0; err[0] = '\0';
                ImGui::CloseCurrentPopup();
            }
        }
        ImGui::SameLine();
...
{
    if (bank_state::open_transfer) {
        ImGui::OpenPopup("Transfer");
        bank_state::open_transfer = false;
    }
    ...
    if (src >= 0 && src < (int)bank_state::clients.size()) {
        ...
        ImGui::Text("To:");     ImGui::SameLine(90.f);
        ImGui::SetNextItemWidth(210.f);
        const char* preview = (target_idx >= 0 && target_idx < (int)bank_state::clients.size())
            ? bank_state::clients[target_idx].getName().c_str()
            : "Select recipient...";
        if (ImGui::BeginCombo("##tr_to", preview)) {
            for (int i = 0; i < (int)bank_state::clients.size(); ++i) {
                if (i == src) continue;
                if (ImGui::Selectable(bank_state::clients[i].getName().c_str(),
                    target_idx == i))
                    target_idx = i;
            }
            ImGui::EndCombo();
        ...
        if (ImGui::Button("Transfer", { 100.f,0.f })) {
            if (target_idx < 0 || target_idx >= (int)bank_state::clients.size())
                ...
                amount = 200.0; target_idx = -1; err[0] = '\0';
                ImGui::CloseCurrentPopup();
            }
        }
        ImGui::SameLine();
...
//{
//    if (bank_state::open_add_employee) {
//        ImGui::OpenPopup("Add Employee");
//        bank_state::open_add_employee = false;
//    }
...
//    static char name_buf[64] = {};
//    static char pass_buf[64] = {};
//    static char role_buf[48] = {};
//    static double salary = 5000.0;
//    static char   err[256] = {};
//
//    ImGui::TextDisabled("Register a new bank employee");
//    ImGui::Separator(); ImGui::Spacing();
//
//    ImGui::Text("Full Name:"); ImGui::SameLine(120.f);
//    ImGui::SetNextItemWidth(210.f);
//    ImGui::InputText("##e_name", name_buf, sizeof name_buf);
//
//    ImGui::Text("Password:"); ImGui::SameLine(120.f);
//    ImGui::SetNextItemWidth(210.f);
//    ImGui::InputText("##e_pass", pass_buf, sizeof pass_buf,
//        ImGuiInputTextFlags_Password);
//
//    ImGui::Text("Role:");     ImGui::SameLine(120.f);
//    ImGui::SetNextItemWidth(210.f);
//    ImGui::InputText("##e_role", role_buf, sizeof role_buf);
//
...
//            strncpy_s(err, sizeof err, "Invalid password — 8-20 chars, no spaces.", _TRUNCATE);
//        else if (r.empty())
//            strncpy_s(err, sizeof err, "Role cannot be empty.", _TRUNCATE);
//        else if (!utils::Validation::validateSalary(salary))
//            strncpy_s(err, sizeof err, "Salary is below the minimum ($5000).", _TRUNCATE);
//        else {
//            bank_state::employees.emplace_back(n, p, r, salary);
//            memset(name_buf, 0, sizeof name_buf);
//            memset(pass_buf, 0, sizeof pass_buf);
//            memset(role_buf, 0, sizeof role_buf);
...
//{
//    if (bank_state::open_set_salary) {
//        ImGui::OpenPopup("Update Salary");
//        bank_state::open_set_salary = false;
//    }
//    if (!ImGui::BeginPopupModal("Update Salary", nullptr,
//        ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove)) return;
//
//    static double new_sal = 5000.0;
//    static char   err[128] = {};
...
//        ImGui::Separator(); ImGui::Spacing();
//        ImGui::Text("New Salary:"); ImGui::SameLine(110.f);
//        ImGui::SetNextItemWidth(170.f);
...
//            if (!utils::Validation::validateSalary(new_sal))
//                strncpy_s(err, sizeof err, "Salary below minimum ($5000).", _TRUNCATE);
//            else {
//                e.setSalary(new_sal);
...
//{
//    if (bank_state::open_add_admin) {
//        ImGui::OpenPopup("Add Administrator");
//        bank_state::open_add_admin = false;
//    }
...
//    ImGui::Text("Full Name:"); ImGui::SameLine(120.f);
//    ImGui::SetNextItemWidth(210.f);
//    ImGui::InputText("##a_name", name_buf, sizeof name_buf);
//
//    ImGui::Text("Password:"); ImGui::SameLine(120.f);
//    ImGui::SetNextItemWidth(210.f);
//    ImGui::InputText("##a_pass", pass_buf, sizeof pass_buf,
//        ImGuiInputTextFlags_Password);
//
//    if (err[0]) {
//        ImGui::Spacing();
...
//            memset(pass_buf, 0, sizeof pass_buf);
//            err[0] = '\0';
//            bank_state::set_status("Administrator added.");
//            ImGui::CloseCurrentPopup();
//        }
//    }
//    ImGui::SameLine();
...
static void view_clients(float w, float h)
{
    ...
    stats_bar("Total Clients:", std::to_string(clients.size()).c_str(),
        { 0.40f,0.70f,1.f,1.f },
        "Total on Deposit:", fmt_money(total_bal).c_str(),
        { 0.30f,0.85f,0.50f,1.f }, w);
    ImGui::Spacing();
        ...
        ImGuiTableFlags_ScrollY | ImGuiTableFlags_SizingStretchProp))
    {
        ...
        for (int i = 0; i < (int)clients.size(); ++i) {
            ...
            // ID + selectable spanning all columns
            ImGui::TableSetColumnIndex(0);
            char sel_id[24];
            snprintf(sel_id, sizeof sel_id, "%d##cs%d", clients[i].getID(), i);
            if (ImGui::Selectable(sel_id, sel,
                ImGuiSelectableFlags_SpanAllColumns |
                ImGuiSelectableFlags_AllowOverlap, { 0, 24.f }))
                selected_client = i;
            ...
            ImVec4 bcol = (bal < 1500.0)
                ? ImVec4{ 1.f,0.40f,0.40f,1.f }
            : ImVec4{ 0.30f,0.85f,0.50f,1.f };
    ...
    // Deferred delete (safe — outside the table loop)
    if (delete_idx >= 0) {
        clients.erase(clients.begin() + delete_idx);
        if (selected_client >= (int)clients.size())
            selected_client = (int)clients.size() - 1;
    }
...
//    stats_bar("Total Employees:", std::to_string(employees.size()).c_str(),
//        { 0.40f,0.70f,1.f,1.f },
//        "Monthly Payroll:", fmt_money(total_payroll).c_str(),
//        { 1.f,0.76f,0.30f,1.f }, w);
//    ImGui::Spacing();
//
//    // ── Employee table ────────────────────────────────────────
//    float table_h = h - 168.f;
...
//            ImGui::TableNextRow(ImGuiTableRowFlags_None, 26.f);
//            bool sel = (selected_employee == i);
//
//            ImGui::TableSetColumnIndex(0);
//            char sel_id[24];
//            snprintf(sel_id, sizeof sel_id, "%d##es%d", employees[i].getID(), i);
//            if (ImGui::Selectable(sel_id, sel,
//                ImGuiSelectableFlags_SpanAllColumns |
//                ImGuiSelectableFlags_AllowOverlap, { 0, 24.f }))
//                selected_employee = i;
//
//            ImGui::TableSetColumnIndex(1);
//            ImGui::Text("%s", employees[i].getName().c_str());
//
//            ImGui::TableSetColumnIndex(2);
//            ImGui::TextDisabled("%s", employees[i].getRole().c_str());
//
//            ImGui::TableSetColumnIndex(3);
...
//
//    if (delete_idx >= 0) {
//        employees.erase(employees.begin() + delete_idx);
//        if (selected_employee >= (int)employees.size())
//            selected_employee = (int)employees.size() - 1;
//    }
//
//    // ── Toolbar ───────────────────────────────────────────────
//    ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();
...
//
//    // ── Employee roster (read-only quick-view) ────────────────
//    ImGui::Spacing(); ImGui::Spacing();
//    ImGui::PushStyleColor(ImGuiCol_Text, { 0.65f,0.78f,1.f,1.f });
//    ImGui::SetWindowFontScale(1.05f);
//    ImGui::Text("Employee Roster");
//    ImGui::SetWindowFontScale(1.f);
//    ImGui::PopStyleColor();
//    ImGui::Separator(); ImGui::Spacing();
//
...
bool banking_gui::init(HWND hwnd, ID3D11Device* dev, ID3D11DeviceContext* ctx)
{
    ...
    s.PopupRounding = 4.f;   s.ScrollbarRounding = 4.f;
    ...
    // ----- Seed demo data -----
    bank_state::clients.emplace_back("Alice Smith", "password001");
    bank_state::clients.back().setBalance(8500.0);
    bank_state::clients.emplace_back("Bob Johnson", "password002");
    bank_state::clients.back().setBalance(22000.0);
    bank_state::clients.emplace_back("Carol White", "password003");
    bank_state::clients.back().setBalance(3200.0);
    bank_state::clients.emplace_back("Daniel Brown", "password004");
    bank_state::clients.back().setBalance(15750.0);
...
void banking_gui::tick()
{
    ...
    if (bank_state::status_timer > 0.f) {
        ImGui::SetCursorPosY(display.y - 88.f);
        ImGui::Separator(); ImGui::Spacing();
        float alpha = (bank_state::status_timer < 1.f) ? bank_state::status_timer : 1.f;
        ImVec4 tcol = bank_state::status_is_error
